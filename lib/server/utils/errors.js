...
export class CsrfError extends AppError {
  constructor(message = "CSRF check failed") {
    super(message, { status: 403, code: "CSRF_FAILED" });
    this.name = "CsrfError";
  }
}

// A storage file exists but could not be parsed.
//
// This deliberately fails the request rather than falling back to an empty
// collection. Silently treating a corrupt consumed-txhashes.json as "no hashes
// consumed" would make every previously claimed TX hash claimable again —
// turning a disk problem into a double-spend. Refusing to serve is the safe
// failure here; an operator restores from the .bak and restarts.
export class StorageCorruptError extends AppError {
  constructor(filePath, cause) {
    super(`Storage file is corrupt: ${filePath}`, { status: 500, code: "STORAGE_CORRUPT" });
