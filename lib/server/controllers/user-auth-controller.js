...
export async function userLoginController(request) {
  ...
  cookies().set(COOKIE_NAMES[ROLES.USER], session.token, cookieOptions(ROLES.USER, session.maxAge));

  return json({ ok: true, user: { id: user.id, username: user.username, email: user.email } });
}

export async function userLogoutController() {
  const token = cookies().get(COOKIE_NAMES[ROLES.USER])?.value;

  await destroySession(token);
  cookies().set(COOKIE_NAMES[ROLES.USER], "", clearedCookieOptions(ROLES.USER));
