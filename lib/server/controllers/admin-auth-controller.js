...
export async function adminLoginController(request) {
  ...
  (await cookies()).set(COOKIE_NAMES[ROLES.ADMIN], session.token, cookieOptions(ROLES.ADMIN, session.maxAge));
...
export async function adminLogoutController() {
  const token = (await cookies()).get(COOKIE_NAMES[ROLES.ADMIN])?.value;

  // Delete the session file too — clearing the cookie alone would leave a
  // token that still works if it was captured.
  await destroySession(token);
  (await cookies()).set(COOKIE_NAMES[ROLES.ADMIN], "", clearedCookieOptions(ROLES.ADMIN));
