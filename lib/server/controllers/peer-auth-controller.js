...
export async function peerLoginController(request) {
  ...
  (await cookies()).set(COOKIE_NAMES[ROLES.PEER], session.token, cookieOptions(ROLES.PEER, session.maxAge));
...
export async function peerLogoutController() {
  const token = (await cookies()).get(COOKIE_NAMES[ROLES.PEER])?.value;

  await destroySession(token);
  (await cookies()).set(COOKIE_NAMES[ROLES.PEER], "", clearedCookieOptions(ROLES.PEER));
