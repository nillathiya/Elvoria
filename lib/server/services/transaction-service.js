...
import { TX_STATUS, REASONS, AppError, ValidationError, NotFoundError } from "../utils/errors.js";
...
export async function verifyTxHash({ peerId, methodId, txHash }) {
  ...
  return withTxHashLock(normalized.registryKey, async () => {
    ...
    if (!addresses.length) {
      // Not the peer's fault and not something they can fix, so say so rather
      // than leaving them to guess at their own hash.
      throw new AppError(
        "This method has no receiving address configured yet. Ask an administrator to add one.",
        { status: 503, code: "NO_ACTIVE_ADDRESS" }
      );
    }
