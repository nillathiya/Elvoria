...
// Support WhatsApp, read from the environment so the real number stays out of
// source/git (set it in .env.local). NEXT_PUBLIC_ because config.js is imported
// by client components too; the value is public anyway (it's shown on the page).
...
// Unset → the WhatsApp channel is simply omitted (see lib/legalDocs.js).
