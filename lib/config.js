...
// Support WhatsApp — the single number every "contact us" / support surface
// dials, plus the floating chat button. An env var can override it per
// deployment, but the account number is the built-in default so the channel is
// wired in out of the box.
// SUPPORT_WHATSAPP is digits-only in international format — it feeds the wa.me
// deep link, which rejects spaces, "+" or dashes. _DISPLAY is what the UI shows.
export const SUPPORT_WHATSAPP = process.env.NEXT_PUBLIC_SUPPORT_WHATSAPP || "17856828854";
export const SUPPORT_WHATSAPP_DISPLAY =
  process.env.NEXT_PUBLIC_SUPPORT_WHATSAPP_DISPLAY || "+1 785 682 8854";

// Builds a wa.me deep link, optionally pre-filling the chat with a message.
// Returns "" when no number is configured so callers can hide the channel.
export function whatsappHref(message = "") {
  if (!SUPPORT_WHATSAPP) return "";
  const query = message ? `?text=${encodeURIComponent(message)}` : "";
  return `https://wa.me/${SUPPORT_WHATSAPP}${query}`;
}
