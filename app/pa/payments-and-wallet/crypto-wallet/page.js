...
//
// This page previously shipped four live third-party wallets here — the ERC20
// one had sent 7.7 million transactions and the ETH one holds tens of
// thousands of ETH; both are exchange wallets. They were rendered as a QR code
// with a copy button under "Scan to deposit", which is not inert demo data
// like a fake balance is: it is an instruction, and a wallet app will carry it
// out. Anyone who scanned it would have paid a stranger, irreversibly.
//
// These four are provably nobody's wallet: each is the 0x…dead burn account in
// its network's own encoding. They render identically, so the demo is
// unchanged, but no third party can be paid by mistake.
//
// The real deposit flow is /pa/deposit, which serves addresses the admin
// configured and verifies transactions against the chain. This page shows none
// of that and never should.
const networks = [
  { value: "btc", label: "Bitcoin", min: "0.0005 BTC", address: "1BitcoinEaterAddressDontSendf59kuE" },
  { value: "usdt-trc20", label: "Tether TRC20", min: "10 USDT", address: "T9yD14Nj9j7xAB4dbGeiX9h8upfCg3PBbY" },
  { value: "usdt-erc20", label: "Tether ERC20", min: "20 USDT", address: "0x000000000000000000000000000000000000dEaD" },
        ...
        <Card className={styles.receiveCard}>
          ...
          {tab === "deposit" ? (
            <div className={styles.depositBody}>
              ...
              {/* The address above is a burn address and this page is a
                  mockup, so it must not read as a live deposit slip. */}
              <p className={styles.note}>
                <strong>Demo page.</strong> The address above is a burn address and cannot be
                withdrawn from by anyone — funds sent to it are destroyed. To make a real
                deposit, use <Link href="/pa/deposit">Deposit</Link>.
              </p>
            </div>
          ) : (
